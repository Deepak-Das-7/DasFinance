export const Colors = {
  // Primary Colors
  primary: "rgb(0, 110, 255)", // Facebook Blue
  secondary: "rgb(28, 28, 28)", // Dark Mode Elements
  accent: "rgb(201, 53, 127)", // Instagram Pink

  // Background Colors
  background: {
    light: "rgb(255, 255, 255)",
    dark: "rgb(0, 0, 0)",
    gray: "rgb(161, 161, 161)", // Light Gray for Sections
    blur: "rgba(0, 0, 0, 0.5)", // Dark Overlay for Modals
  },

  // Text Colors
  text: {
    light: "rgb(17, 24, 28)",
    dark: "rgb(225, 232, 237)",
    muted: "rgb(104, 112, 118)",
    white: "rgb(255, 255, 255)",
    black: "rgb(0, 0, 0)",
  },

  // Tint Colors (Branding Highlights)
  tint: {
    light: "rgb(24, 119, 242)",
    dark: "rgb(131, 58, 180)", // Instagram Purple
    highlight: "rgb(255, 204, 0)", // Warning Yellow
  },

  // Icon Colors
  icon: {
    light: "rgb(104, 112, 118)",
    dark: "rgb(170, 184, 194)",
    active: "rgb(24, 119, 242)",
    inactive: "rgb(104, 112, 118)",
  },

  // Tab Navigation Colors
  tabIcon: {
    default: "rgb(104, 112, 118)",
    selected: "rgb(24, 119, 242)",
  },

  // Button Colors
  button: {
    primary: "rgb(0, 122, 255)",
    secondary: "rgb(28, 28, 28)",
    disabled: "rgb(192, 192, 192)",
    danger: "rgb(255, 59, 48)",
  },

  // Status Colors
  status: {
    success: "rgb(52, 199, 89)",
    error: "rgb(252, 59, 48)",
    warning: "rgb(255, 204, 0)",
    info: "rgb(0, 122, 255)",
  },

  // Gradients
  gradients: {
    instagram: ["rgb(255, 105, 180)", "rgb(255, 80, 90)", "rgb(131, 58, 180)"],
    sunset: ["rgb(255, 126, 95)", "rgb(253, 38, 125)"],
    ocean: ["rgb(54, 209, 220)", "rgb(91, 134, 229)"],
    fire: ["rgb(255, 81, 47)", "rgb(221, 36, 118)"],
  },

  // Shadows & Borders
  shadow: {
    light: "rgba(0, 0, 0, 0.1)",
    dark: "rgba(0, 0, 0, 0.3)",
  },

  border: {
    light: "rgb(225, 232, 237)",
    dark: "rgb(47, 51, 54)",
    primary: "rgb(24, 119, 242)",
  },

  // Additional UI Colors
  notification: "rgb(255, 59, 48)", // Red Badge
  overlay: "rgba(0, 0, 0, 0.5)", // Dark Overlay for Modals
  placeholder: "rgb(170, 184, 194)", // Input Placeholder Text
};
